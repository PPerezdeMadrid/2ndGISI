...
**La principal aplicación de ANOVA es cuando queremos usar test post-hoc. Si conlos contrastes basta, ANOVA no aporta demasiado sobre los análisis de regresión ya hechos.** 
...
En el caso de modelos ANOVA, existen varios tamaños de efecto. Entre los mas conocidos están eta-squared y omega-squared. Para complicar aún más las cosas, las heurísticas
...
como significativa. Esto lo logramos con distintos métodos de **ajuste de p-valores**. Fíjate que el test omnibus sirve como una primera barrera protectora antes de  lanzarnos a hacer **comparaciones múltiples**
```{r}
...
Entre los métodos de ajuste, podemos distinguir entre 
  ...
  2. Métodos centrados en controlar el **false discovery rate (FDR)**, que se  corresponde con el credo (considerablemente más optimista) 
  "vamos a intentar estimar cuántos errores tipo I cometo y a no pasarme de cierto número (pero no pasa nada si hay algún error)".
...
summary(anxiety_lm)
  #-2.14 + pretest*1 + group*1 + group2*0.56

# 4) Posthoc tests!
  #comparar los niveles de POSTTEST con los GRUPOS
pairwise.t.test(anxiety$posttest, anxiety$group, p.adjust.method = "bonferroni") #NO NOS SIRVE
...
teniendo en cuenta además el sexo de los pacientes que toman el medicamento. Supongamos que  el efecto de las drogas y edad se mide  en términos de reducción del tiempo de reacción a algún estímulo y que se obtienen los resultados del fichero "drugs_1.csv". Visualiza el efecto de las drogas y sexo en los tiempos de reacción
y propón un modelo.
